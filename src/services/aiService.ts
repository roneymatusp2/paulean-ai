...
class AdvancedAIService {
  ...
  private selectOptimalProvider(request: AIRequest): keyof typeof AI_PROVIDERS {
    ...
    if (category === 'Planning' || category === 'Assessment' || toolName.includes('Lesson') || toolName.includes('Curriculum')) {
      const educationProviders: (keyof typeof AI_PROVIDERS)[] = ['CLAUDE', 'OPENAI', 'MISTRAL', 'DEEPSEEK']
    ...
    if (category === 'Communication' || toolName.includes('Email') || toolName.includes('Letter')) {
      const commProviders: (keyof typeof AI_PROVIDERS)[] = ['OPENAI', 'CLAUDE', 'MISTRAL', 'DEEPSEEK']
    ...
    if (category === 'Creative' && (toolName.includes('Writing') || toolName.includes('Story') || toolName.includes('Creative'))) {
      const creativeProviders: (keyof typeof AI_PROVIDERS)[] = ['CLAUDE', 'MISTRAL', 'OPENAI', 'DEEPSEEK']
      const provider = this.getBestAvailableProvider(creativeProviders)
      if (provider) return provider
    }

    // 💻 Technical/Coding tasks → Claude first, then DeepSeek (specialized and cost-effective)
    if (toolName.includes('Code') || toolName.includes('Programming') || toolName.includes('Technical') || toolName.includes('Math')) {
      const techProviders: (keyof typeof AI_PROVIDERS)[] = ['CLAUDE', 'OPENAI', 'DEEPSEEK', 'MISTRAL']
      const provider = this.getBestAvailableProvider(techProviders)
      if (provider) return provider
    }

    // 📊 Data analysis → Claude first for quality, then others
    if (category === 'Data' || toolName.includes('Analysis') || toolName.includes('Research')) {
      const dataProviders: (keyof typeof AI_PROVIDERS)[] = ['CLAUDE', 'OPENAI', 'DEEPSEEK', 'MISTRAL']
      const provider = this.getBestAvailableProvider(dataProviders)
      if (provider) return provider
    }

    // 💰 Cost-sensitive or bulk operations → Still prioritize quality: Claude → OpenAI → Mistral → DeepSeek
    if (priority === 'low') {
      const costEffectiveProviders: (keyof typeof AI_PROVIDERS)[] = ['CLAUDE', 'OPENAI', 'MISTRAL', 'DEEPSEEK']
      const provider = this.getBestAvailableProvider(costEffectiveProviders)
      if (provider) return provider
    }

    // 🎯 Default: ALWAYS prioritize quality - Claude → OpenAI → Mistral → DeepSeek (NEVER DeepSeek first!)
    const defaultProviders: (keyof typeof AI_PROVIDERS)[] = ['CLAUDE', 'OPENAI', 'MISTRAL', 'DEEPSEEK', 'TOPMEDIA']
    const provider = this.getBestAvailableProvider(defaultProviders)
    
    console.log(`🎯 Selected provider: ${provider || 'CLAUDE_FALLBACK'}`)
    return provider || 'CLAUDE' // Ultimate fallback to Claude, not DeepSeek
  }
